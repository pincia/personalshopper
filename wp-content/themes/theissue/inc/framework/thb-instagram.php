...
function thb_getInstagramPhotos_json( $number = 6, $username, $access_token ) {
	...
	// Check Access Token
	if ( ! $access_token ) {
		$result['error'] = esc_html__( 'Please enter an Instagram Access Token.', 'theissue' );
		return $result;
	}
	// Check Username
	if ( ! $username ) {
		$result['error'] = esc_html__( 'Please enter an Instagram User.', 'theissue' );
		return $result;
	}

	if ( ! defined ( 'THB_DEMO_SITE' ) && '8730733905.1677ed0.08da52b798f549c395fb4bfb4cb740e5' === $access_token ) {
		$result['error'] = esc_html__( 'Please enter your own Instagram Token.', 'theissue' );
		return $result;
	}
	...
	if ( ! $result ) {
		...
		if ( $user->meta->error_message ) {
			$result['error'] = $user->meta->error_message;
			return $result;
		}
		...
		set_transient ( $transient_name, $result, HOUR_IN_SECONDS * 2 );
	}
	return $result;
}
function thb_getInstagramPhotos_html( $number = 6, $username ) {
    ...
    $ins_data_json = array_shift( $ins_data );
		...
		if ( ! $ins_data_json ) {
			return esc_html__( 'Instagram html data cannot be retrieved.', 'theissue' );
		}
		$image_data = false;
    ...
    if ( $ins_data_json ) {
      $instagram_json = json_decode( $ins_data_json, true );
			...
			if ( ! isset( $instagram_json['entry_data']['ProfilePage'][0]['graphql']['user'] ) ) {
				return esc_html__( 'Instagram data is not set, please check the ID.', 'theissue' );
			}
			...
			if ( ! isset( $thb_user_data['edge_owner_to_timeline_media']['edges'] ) ) {
				return esc_html__( 'There are no images in your account or the account is private.', 'theissue' );
			}
      // Live Count.
      if ( ! empty( $thb_user_data['edge_owner_to_timeline_media']['edges'] ) ) {
        $image_data = $thb_user_data['edge_owner_to_timeline_media']['edges'];
      } else {
        $result['error'] = esc_html__( 'There are no images in your account or the account is private.', 'theissue' );
      }
      ...
      $result['error'] = esc_html__( 'Instagram html data cannot be retrieved.', 'theissue' );
    }
		if ($image_data) {
