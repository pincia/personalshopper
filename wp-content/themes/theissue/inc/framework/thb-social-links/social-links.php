...
class THB_Social_Links {
  ...
  */
  ...
  public function thb_get_youtube( $username ) {
    ...
    $thb_google_api_key = apply_filters( 'thb_google_api_key', ot_get_option( 'social_youtube_api_key' ) );

    if ( defined( 'THB_DEMO_SITE' ) ) {
      $thb_google_api_key = 'AIzaSyDwAwpB3ESvJbPLjhz9HpI4cH2OE8zAaNA';
    }
    // Generate Params
    switch ( $channel_type ) {
      case 'username':
  ...
  public function thb_get_instagram( $username ) {
    ...
    $cache_name = 'thb_social_instagram_' . $username . '_count';
    $data       = get_transient( $cache_name );
    ...
    if ( $access_token && '' !== $access_token ) {
      ...
      if ( isset( $user->data->counts->followed_by ) ) {
  		...
  		} else if ( defined( 'THB_DEMO_SITE' ) ) {
        $result['count'] = 7400;
      } else if ( isset( $user->error_message )  ) {
        $result['error'] = $user->error_message;
      } else if ( isset( $user->meta->error_message )  ) {
        $result['error'] = $user->meta->error_message;
      } else {
        $result['error'] = esc_html__( 'Please check your Instagram Access Token.', 'theissue' );
      }

      if ( $result['count'] ) {
        $this->thb_set_cache( $cache_name, $result );
      }
  ...
  public function thb_get_vimeo( $username ) {
    ...
    // Check username
    if ( ! $username ) {
      $result['error'] = esc_html( 'Please enter Vimeo Channel ID.', 'theissue' );
      return $result;
    }
