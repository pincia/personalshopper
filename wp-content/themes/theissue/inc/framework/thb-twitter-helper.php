...
class ThbTwitterHelper {
	public function thb_getTweetText( $tweet ) {
		...
		if ( $tweet->text ) {
			...
			// hashtags links
			$tweet_text = preg_replace('/\#([a-záéíóúüñçA-ZÁÉÍÓÚÜÑÇ0-9\.\_\-]+)/i', '<a target="_blank" href="https://twitter.com/search?q\=$1">#$1</a>', $tweet_text);
	...
	public function thb_getTweetTime( $tweet ) {
		if ( $tweet->created_at ) {
			return sprintf( esc_html_x( '%s ago', '%s = human-readable time difference',
			'theissue' ), human_time_diff(strtotime( $tweet->created_at), current_time('timestamp') ) );
		} else {
			return '';
		}
	}
